...
namespace SignalRWebUI.Controllers
{
    public class MenuController : Controller
    {
        ...
        public async Task<IActionResult> AddBasket(int id, int menuTableId)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7134/api/Basket", stringContent);

            var client2 = _httpClientFactory.CreateClient();
            //var jsonData2 = JsonConvert.SerializeObject(updateCategoryDto);
            //StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client2.GetAsync("https://localhost:7134/api/MenuTables/ChangeMenuTableStatusToTrue?id=" + menuTableId);
